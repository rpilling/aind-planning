...
class PgNode_s(PgNode):
    ...
    '''
    A planning graph node representing a state (literal fluent) from a planning problem.

    Args:
    ----------
    symbol : str      A string representing a literal expression from a planning problem domain.

    is_pos : bool     Boolean flag indicating whether the literal expression is positive or negative.
    '''
    ...
    def __init__(self, symbol: str, is_pos: bool):
        ...
        Instance variables calculated:
            literal: expr       fluent in its literal form including negative operator if applicable
...
class PlanningGraph():
    ...
    '''
    A planning graph as described in chapter 10 of the AIMA text. The planning graph can be used to reason about 
    '''
    ...
    def __init__(self, problem: Problem, state: str, serial_planning=True):
        ...
        Instance variable calculated:
            ...
            * fs: FluentState (the state represented as positive and negative fluent literal lists)
        ...
        self.serial = serial_planning # only one action at a time 
        self.all_actions = self.problem.actions_list + self.noop_actions(self.problem.state_map) # real action + persistence action
        self.s_levels = [] # state levels Si
        self.a_levels = [] # action levels Ai
        self.create_graph()
    ...
    def noop_actions(self, literal_list):
        ...
        "No-Op" actions are virtual actions (i.e., actions that only exist in the planning graph, not in the planning problem domain) that operate
        on each fluent (literal expression) from the problem domain. No op actions "pass through" the literal expressions from one level of the
        planning graph to the next.

        The no-op action list requires both a positive and a negative action for each literal expression.

        * Positive no-op actions require the literal as a positive precondition and add the literal expression as an effect in the output, and
        * Negative no-op actions require the literal as a negative precondition and remove the literal expression as an effect in the output.
        ...
        for fluent in literal_list:
            ...
            act1 = Action(expr("Noop_pos({})".format(fluent)), ([fluent], []), ([fluent], [])) # Action( expr(), precond(+), effect(+) ) 
            action_list.append(act1)

            act2 = Action(expr("Noop_neg({})".format(fluent)), ([], [fluent]), ([], [fluent])) # Action( expr(), precond(-), effect(-) ) 
    ...
    def create_graph(self):
        ...
        The S0 initial level has been implemented for you.  It has no parents and includes all of the literal fluents that are part of the initial state.
        At the start of a problem planning search, this will be the same as the initial state of the problem.  However, the planning graph can be built
        from any state in the Planning Problem.

        This function should only be called by the class constructor.

        :return: builds the graph by filling s_levels[] and a_levels[] lists with node sets for each level
        ...
        while not leveled:
            ...
            self.add_action_level(level)                # TODO #1: Add an A (action) level to the planning graph (>> def below)
            self.update_a_mutex(self.a_levels[level])

            level += 1
            
            self.add_literal_level(level)               # TODO #2: Add a S (state) level to the planning graph   (>> def below)
    ...
    def add_action_level(self, level):
        ...
        # (1) Define a_nodes and s_nodes at current level

        self.a_levels.append(set())

        s_nodes = self.s_levels[level]
        
        # (2) Determine the actions to add
        ...
        # (3) connect the nodes to the previous S literal level
    ...
    def add_literal_level(self, level):
        ...
        # (1) Add empty set for literal level

        self.s_levels.append(set())

        # (2) Determine what literals to add 
    ...
    def update_a_mutex(self, nodeset):
        ...
        A mutex relation holds between two actions a given level if the planning graph is
        a serial planning graph and the pair are nonpersistence actions or if any of the
        three conditions hold between the pair:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # TODO test for inconsistence between nodes
